...
if( isset($_POST['teamID']) && isset($_POST['jobID']) && isset($_POST['cur_date'])){
	...
	$q = $conn->prepare("UPDATE projects SET template_team = :teamID, job_status = 13 WHERE id = :jobID");
...
<? foreach ($rows as $row){ ?>
		templaters[<?= $row['temp_team_id'] ?>] = '<?= $row['temp_team_name'] ?>';
<? } ?>
	...
	function initializeMap() {
		...
		$('.get_info_btn').click(function(){
			...
			console.log('Marker Started!', value['job_lat'], value['job_long']);
			console.log(latitude);
			console.log(longitude);
			console.log(plotLat);
			console.log(plotLong);
			...
			console.log('Marker Ended!', latitude, longitude);
								...
								(function (marker, value) {
									...
									var html_str = "<div style = 'width:200px;min-height:40px'>";
										...
										if (value['temp_first_stop'] == 1){
											html_str +=   "<p class='text-primary'><b>1st Stop Install</b></p>";
										}else if (value['temp_am'] == 1) {
											html_str +=   "<p class='text-primary'><b>AM Install</b></p>";
										}else if (value['temp_pm'] == 1) {
											html_str +=   "<p class='text-primary'><b>PM Install</b></p>";
										}
										var tempTeamID = value['template_team'];
										html_str +=   "<p>Templater: <b>" + templaters[tempTeamID] + "</b></p>";
										...
										if (selectedTeam == value['template_team']) {
											html_str += 'selected'
										};
					...
					$.each( result, function( key, value ) {
						...
						joblist_str[teamID] += '<li class="list-group-item py-1"><div class="btn btn-primary btn-sm mr-1" onClick="targetMap('+latitude+','+longitude+')"><i class="fas fa-bullseye"></i></div><a class="text-primary text-left btn btn-sm p-0" target="_blank" onClick="' + modalFunct + '">' + value['job_name'] + '<br><span style="color:#555;text-shadow:none;font-weight:normal;cursor:pointer">' + value['address_1'] + ', ' + value['city'] + ', ' + value['state'] + ', ' + value['zip'] + '</span></a></li>';
...
modalPull += '					<div class="row">';
									...
									if (value['temp_first_stop'] == 1){
										modalPull +=   "<p class='text-danger'><b>1st Stop Install</b></p>";
									}else if (value['temp_am'] == 1) {
										modalPull +=   "<p class='text-danger'><b>AM Install</b></p>";
									}else if (value['temp_pm'] == 1) {
										modalPull +=   "<p class='text-danger'><b>PM Install</b></p>";
									}
					...
					<div class="col-12 pr-4 team_section">
						...
						<div class="row team<?= $row['temp_team_id'] ?>">
							...
							<div class="col-sm-9 team_no">
								...
								<? 
								if ($row['temp_team_id'] == 0) {
									echo '<span class="text-primary">Templates: </span>' . $row['temp_team_name'];
								} else {
									echo $row['temp_team_name'];
								}
								?>
								</span>
							</div>
							<div class="col-sm-2">
								...
								if ($row['temp_team_id'] > 0) {
								?>
								<button class="btn btn-primary mt-1 rteBtn" onclick="display_route(<?php echo $row['temp_team_id']; ?>)"><i class="far fa-route"></i></button>
								<?
								}
								?>
							</div>

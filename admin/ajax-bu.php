...
if ($action=="update_inst_dicounts") {
	unset($_POST['action']);
	$pid = $_POST['pid'];
	unset($_POST['pid']);
	$run = new project_action;
	$run -> update_inst_dicounts($pid);
	echo $run;
}
...
if ($action=="get_pieces") {
	...
	foreach($get_pieces->get_pieces($_POST) as $results) {
		$pOptions .= '<option value="' . $results['piece_id'] . '">' . $results['piece_name'] . '</option>';
	}
	echo $pOptions;
}
...
if ($action == "update_install_data") {
	...
	$price_extra = $_POST['price_extra'];
	$install_price = $price_calc + $accs_prices + $price_extra;
...
if ($action=="view_selected_pjt") {
	...
	foreach($search->project_data_fetch($_POST) as $results) {
		...
		if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14) {
			...
			$project_costs = number_format($project_costs, 2, '.', ','); 
			$profit = number_format($profit, 2, '.', ','); 
			$html .= '<div class="row text-center"><h2 class="text-center w-100 text-danger">Costs: $' . $project_costs . ' - Profit: $' . $profit . '</h2></div>'; 
			$html .= '<hr>'; 
		}
	...
	foreach($search->install_data_search($_POST) as $results) {
		?>
			...
			<div class="row w-100 d-flex d-print-flex">
	        	<div class="col-4 col-md-4 text-primary"><div class="d-inline d-print-none" onClick="delete_install(<?= $results['id']; ?>)"> <i class="fas fa-trash text-danger"></i></div><h4 class="d-inline"><?= $results['type_name']; ?> - <?= $results['install_name']; ?> </h4></div>
...
if ($action=="view_selected_inst") {
	...
	foreach($search->install_data_fetch($_POST) as $results) {
		...
		$html .= '	<div class="col-12 col-md-3">Tear Out? <b>' . $results['tear_out'] . '</b></div>';
		...
		$html .= '	<hr>';
		$html .= '	<div class="col-12 col-md-2">Material: <b>' . $results['material'] . '</b></div>';
		$html .= '	<div class="col-12 col-md-4">Color: <b>' . $results['color'] . '</b></div>';
		...
		$html .= '	</div>';
		$html .= '	<div class="col-12 col-md-2">Cost P/SqFt: <b>$' . $results['cpSqFt'] . '</b></div>';
		$html .= '	<hr>';
		...
		$clipboard .= " - Tear-Out: " . $results['tear_out'];
	...
	foreach( $instS->sink_data_fetch($_POST) as $r ) {
		$html .= '	<div class="row">';
		$html .= '		<h4 class="d-inline col-12 text-warning"><i class="fas fa-trash-alt text-danger h6" style="cursor:pointer" onClick="deleteSink(' . $r['sink_id'] . ')"></i> ';
		if ($r['sink_name'] > '') {
			if ($r['piece_name'] > 0) {
				$html .= 'Sink for ' . $r['piece_name'] . '<br>';
				$clipboard .= '&lt;br&gt; --- Sink for piece' . $r['piece_name'];
			} else {
				$html .= 'Sink for job<br>';
				$clipboard .= '&lt;br&gt; --- Sink for piece job';
			}
		} else {
			if ($r['piece_name'] > 0) {
				...
				$clipboard .= '&lt;br&gt; --- Faucet for piece job';
			}
		}
		if ($r['sink_name'] > '') {
			$html .= $r['sink_name'] . '</h4>';

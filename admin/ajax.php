...
if ($action=="adjust_material") {
	$pid = $_POST['pid'];
	$lot = $_POST['lot'];
	$location = $_POST['assigned_material'];
	unset($_POST['action']);
	unset($_POST['staff']);
	$adjust_material = new materials_action;
	echo $adjust_material -> adjust_material($_POST);

	$statement = '';
	if ($lot == '' && $location == '') {
		$statement = "Material assignment has been removed.";
	} else {
		$statement = "Material modified.";
	}
	...
	$_POST['cmt_comment'] = $statement;
	$_POST['cmt_priority'] = 'log';
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
}

if ($action=="enter_tie_mats") {
	unset($_POST['action']);
	$enter_tie_mats = new materials_action;
	echo $enter_tie_mats -> enter_tie_mats($_POST);
}

// DISPLAY ITEMS FOR CATEGORY
if ($action=="get_mats_available") {
	unset($_POST['action']);
	$category = $_POST['category'];
	$get_mats_available = new materials_action;
	$array = $get_mats_available->get_mats_available($category);
	$options = '<option value="" disabled selected>Choose Inventory</option>';
	foreach($array as $result) {
		$options .= '<option value="' . $result['item_id'] . '"';
		if ($result['image'] > "") {
			$options .= ' data-icon="../inventory/images/' . $result['image'] . '" ';
		}
		$options .= " price='" . $result['price'] . "' cost='" . $result['cost'] . "'>";
		if ($result['remnant'] == 1) {
			$options .= 'Partial: ';
		}
		$options .= $result['item_name'];
		if ($result['lot'] > "") {
			$options .= ' - Lot: ' . $result['lot'];
		}
		$options .= ' - h: ' . $result['height'] . ' x w: ' . $result['width'];
		if ($result['height_l'] > 0) {
			$options .= ' - L-Shape h: ' . $result['height_l'] . ' x w: ' . $result['width_l'];
		}
		if ($result['sqft'] > 0) {
			$options .= ' - ' . $result['sqft'] . ' sqft';
		}
		if ($result['company'] > "") {
			$options .= ' - ' . $result['company'];
		}
		$options .= '</option>';
	}
	echo $options;
...
// DISPLAY CATEGORIES BY CATEGORY TYPE
if ($action=="get_inv_cats") {
	unset($_POST['action']);
	$category_type = $_POST['category_type'];
	$get_inv_cats = new materials_action;
	$array = $get_inv_cats->get_inv_cats($category_type);
	$options = '<option value="" disabled selected>Choose category</option>';
	foreach($array as $result) {
		$options .= "<option value='" . $result['id_key'] . "'  price='" . $result['price'] . "' cost='" . $result['cost'] . "' item_iname='" . $result['category_name'] . "' item_model='" . $result['category_id'];
		if ($result['image'] > "") {
			$options .= ' data-icon="../inventory/images/' . $result['image'] . '" ';
		}
		$options .= "'>";
		if ($result['category_type'] > 4) { 
			$options .= $result['category_id'] . ' - ';
		}
		$options .= $result['category_name'] . '</option>';
	}
	echo $options;
}
...
if ($action=="jobson_list") {
	...
	foreach($get_jobson->get_jobson() as $results) {
		...
		$html .= '			<div class="progress-bar progress-bar-striped bg-success" role="progressbar" aria-valuenow="'.$progPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$progPct.'%">&ensp;' . $prog . ' Programming</div>';
		$html .= '			<div class="progress-bar progress-bar-striped bg-warning text-left text-nowrap" role="progressbar" aria-valuenow="'.$matPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$matPct.'%; overflow:hidden">&ensp;' . $mat . ' Materials</div>';
		$html .= '			<div class="progress-bar progress-bar-striped blue-gradient text-left text-nowrap" role="progressbar" aria-valuenow="'.$sawPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$sawPct.'%; overflow:hidden">&ensp;' . $saw . ' Saw</div>';
		$html .= '			<div class="progress-bar progress-bar-striped peach-gradient text-left text-nowrap" role="progressbar" aria-valuenow="'.$cncPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$cncPct.'%; overflow:hidden">&ensp;' . $cnc . ' CNC</div>';
		$html .= '			<div class="progress-bar progress-bar-striped purple-gradient text-left text-nowrap" role="progressbar" aria-valuenow="'.$polPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$polPct.'%; overflow:hidden">&ensp;' . $pol . ' Polishing</div>';
		$html .= '			<div class="progress-bar progress-bar-striped" role="progressbar" aria-valuenow="'.$instPct.'" aria-valuemin="0" aria-valuemax="100" style="width:'.$instPct.'%; overflow:hidden">&ensp;' . $inst . ' Installs</div>';
		$html .= '		</div>';
		$html .= '	</div>';
		$today = date('Y-m-d');
		...
		if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14 || $_SESSION['id'] == 1444) {
			...
			if (is_nan($counts['install_sqft_count']) || $counts['install_sqft_count'] == 0) {
				$installPercent = 0;
			} else {
			...
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $progPercent . '%  of today\'s Programming Complete">
								<div class="progress-bar purple-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $progPercent . '" aria-valuemin="0" aria-valuemax="100" style="cursor:context-menu; width:' . $progPercent . '%">&ensp;' . $progPercent . '% - Programmed</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $sawPercent . '%  of today\'s Cutting Complete">
								<div class="progress-bar aqua-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $sawPercent . '" aria-valuemin="0" aria-valuemax="100" style="cursor:context-menu; width:' . $sawPercent . '%">&ensp;' . $sawPercent . '% - Saw</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $cncPercent . '%  of today\'s CNC Complete">
								<div class="progress-bar peach-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $cncPercent . '" aria-valuemin="0" aria-valuemax="100" style="cursor:context-menu; width:' . $cncPercent . '%">&ensp;' . $cncPercent . '% - CNC</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $polishPercent . '%  of today\'s Polishing Complete">
								<div class="progress-bar blue-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $polishPercent . '" aria-valuemin="0" aria-valuemax="100" style="cursor:context-menu; width:' . $polishPercent . '%">&ensp;' . $polishPercent . '% - Polished</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $installPercent . '%  of today\'s Installs Complete">
								<div class="progress-bar lee-made-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $installPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $installPercent . '%">&ensp;' . $installPercent . '% - Installed</div>
							</div>
						</div>';
			$html .= '</div>';
		}
		...
		if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14 || $_SESSION['id'] == 1444 || $_SESSION['id'] == 13) {
			...
			$html .= '<div class="row m-0 p-0">';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="' . $monPercent . '% of next Mondays Schedule Full">
								<div class="progress-bar purple-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $monPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $monPercent . '%">&ensp;' . $monPercent . '% Mon</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="&emsp;' . $tuePercent . '% of next Tuesday\'s Schedule Full">
								<div class="progress-bar aqua-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $tuePercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $tuePercent . '%">&ensp;' . $tuePercent . '% Tue</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="&emsp;' . $wedPercent . '% of next Wednesday\'s Schedule Full">
								<div class="progress-bar peach-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $wedPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $wedPercent . '%">&ensp;' . $wedPercent . '% Wed</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="&emsp;' . $thuPercent . '% of next Thursday\'s Schedule Full">
								<div class="progress-bar blue-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $thuPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $thuPercent . '%">&ensp;' . $thuPercent . '% Thur</div>
							</div>
						</div>';
			$html .= '	<div class="col m-0 p-0">
							<div class="progress w-100 d-print-none m-0 p-0" title="&emsp;' . $friPercent . '% of next Friday\'s Schedule Full">
								<div class="progress-bar lee-made-gradient text-left text-nowrap" role="progressbar" aria-valuenow="' . $friPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $friPercent . '%">&ensp;' . $friPercent . '% Fri</div>
							</div>
						</div>';
			$html .= '</div>';
...
if ($action=="sink_hold") {
	...
	$job = '';
	$uid = '';
	$change_sink_hold = new project_action;
	foreach( $change_sink_hold -> get_sink_update($_POST) as $r ) {
		$repEmail = $r['rep_email'];
		$repName = $r['rep_name'];
		$sinkEmail = $r['sink_email'];
	...
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'Accessory Placed on Hold - ' . $reason;
	$_POST['cmt_priority'] = 911;
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
	...
	$EmailTo = $repEmail;
	$Subject = 'Accessory placed on Hold - Order: ' . $oNum . ' - ' . $job;
	$Body = '<h2> <a target="_blank" href="mailto:' . $sinkEmail . '">' . $sinkName . ' - ' . $sinkEmail . '</a> placed <b>' . $job . '</b> job on hold because of accessory.</h2>';
	$Body .= '<h3>The reason given was: ' . $reason . '</h3>';
	$Body .= '<h2><a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '">' . $job . '</a></h2>';
	$Body .= '<h2><a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '">Quote #: ' . $qNum . ' - Order #:' . $oNum . '</a></h2>';
	$Body .= '<a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '"><button>View Project</button></a>';
...
if ($action=="sink_release") {
	...
	$uid = '';
	$change_sink_hold = new project_action;
	foreach( $change_sink_hold -> get_sink_release($_POST) as $r ) {
		$repEmail = $r['rep_email'];
		$repName = $r['rep_name'];
		$sinkEmail = $r['sink_email'];
		$sinkName = $r['sink_name'];
	...
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'Accessory Released from Hold - ' . $reason;
	$_POST['cmt_priority'] = 911;
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
	...
	$EmailTo = 'khuntington@amanzigranite.com';
	$Subject = 'Accessory Released from Hold - Order: ' . $oNum . ' - ' . $job;
	$Body = '<h2> <a target="_blank" href="mailto:' . $sinkEmail . '">' . $sinkName . ' - ' . $sinkEmail . '</a> Released <b>' . $job . '</b> job from hold.</h2>';
	$Body .= '<h3>The reason given was: ' . $reason . '</h3>';
	$Body .= '<h2><a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '">' . $job . '</a></h2>';
	$Body .= '<h2><a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '">Quote #: ' . $qNum . ' - Order #:' . $oNum . '</a></h2>';
	$Body .= '<a href="https://amanziportal.com/admin/projects.php?edit&pid=' . $pid . '&uid=' . $uid . '"><button>View Project</button></a>';
	...
	foreach( $get_rtos -> get_rtos($_POST) as $r ) {
		...
		if ($r['approval'] == 1) {
			...
			if ( $_POST['type'] == 'personal' || $r['staff'] == $_SESSION['id'] ) {
				...
				$html .= '<div class="col-3 btn btn-lg btn-success" onClick="approveRTO(' . $r['id'] . ',' . $_SESSION['id'] . ')"><i class="fas fa-check"></i> Approve</div>';
			}
		}
...
if ($action=="assign_material_bulk") {
	unset($_POST['action']);
	$run = new materials_action;
	echo $run -> assign_material_bulk($_POST);
}

if ($action=="ordered_material_bulk") {
	unset($_POST['action']);
	$run = new materials_action;
	echo $run -> ordered_material_bulk($_POST);
}

if ($action=="ordered_sink") {
	$pid = $_POST['pid'];
	$sinkName = $_POST['sinkName'];
	$sinkDate = $_POST['sink_date'];
	unset($_POST['sinkName']);
	unset($_POST['action']);
	$run = new materials_action;
	$run -> ordered_sink($_POST);

	$_POST = array();
	$_POST['cmt_ref_id'] = $pid;
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'Accs Ordered. Est. Delivery date: ' . $sinkDate . ' - ' . $sinkName;
	$_POST['cmt_priority'] = 'log';
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
	echo $log;
...
if ($action=="assign_sink") {
	$pid = $_POST['pid'];
	$sinkName = $_POST['sinkName'];
	$assigned_sink = $_POST['assigned_sink'];
	unset($_POST['sinkName']);
	unset($_POST['pid']);
	unset($_POST['action']);
	$run = new materials_action;
	$run -> assign_sink($_POST);
	$run = new materials_action;
	$run -> no_sink($_POST);
	$_POST = array();
	$_POST['cmt_ref_id'] = $pid;
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'On-hand ' . $sinkName . ' - ' . $assigned_sink;
	$_POST['cmt_priority'] = 'log';
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
}
...
if ($action=="no_sink") {
	...
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'No pull needed for ' . $sinkName;
	$_POST['cmt_priority'] = 'log';
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
}
...
if ($action=="sink_reset") {
	...
	unset($_POST['iname']);
	$run = new materials_action;
	$run -> sink_reset($_POST);
	$_POST = array();
	$_POST['cmt_ref_id'] = $pid;
	$_POST['cmt_type'] = 'pjt';
	$_POST['cmt_user'] = $_SESSION['id'];
	$_POST['cmt_comment'] = 'Accessory reset for ' . $aname;
	$_POST['cmt_priority'] = 'log';
	$log_project = new log_action;
	$log = $log_project -> pjt_changes($_POST);
}
...
if ($action=="get_materials_needed") {
	...
	echo $first_tab . ':::' . $second_tab . ':::' . $third_tab;
}

if ($action=="get_accessories_needed") {
	unset($_POST['action']);
	$results = "";
	$search = new materials_action;
	$tmp = array();
	//Get the projects between with job_status > 10 and job_status < 50
	$pjts = $search->get_accessories();
	...
	function show_pjts($result, $status) {
		$accs_name = '';
		if ($result['sink_name'] != "" ) {
			$accs_name = htmlspecialchars($result['sink_name']);
		} else {
			$accs_name = htmlspecialchars($result['faucet_name']);
		}
		$tmp_arr = '
		<hr>
		<div class="container d-md-flex">
			<div class="col-md-12">
				<strong>' . $accs_name . '</strong>
			</div>
		</div>';
		if ($result['sink_hold'] == 1) {
			$tmp_arr .= '
				<hr>
				<div class="container d-flex">
					<div class="col-12 text-danger">
						<b>ACCESSORY ON HOLD</b>
						<div class="btn btn-sm btn-danger mr-2 float-right" onclick="sink_release_modal(' . $_SESSION['id'] .',' . $result['sink_id'] . ',' . $result['pid'] . ')">
							Release Hold <i class="fas fa-ban"></i>
						</div>
					</div>
				</div>';
		} else {
			if ($result['sink_status'] < 2) {
				$tmp_arr .= '
				<hr>
				<div class="container d-flex">
					<div class="col-12 text-danger">
			...
			} else if ($result['sink_status'] == 2) {
				$tmp_arr .= '
				<hr>
				<div class="container d-flex">
					<div class="col-5 text-success">
						<b>'. $status .date("Y-m-d",strtotime($result['sink_date'])).'</b>
					</div>
					<div class="col-5">
						Reference: '.$result['assigned_sink'].'
					</div>
				</div>
				<div class="container d-flex">
					<div class="col-12">
			...
			} else if ($result['sink_status'] == 3) {
				$tmp_arr .= '
				<hr>
				<div class="container d-flex">
					<div class="col-5 text-primary">
						<b>Status: Accessory On Hand</b>
					</div>
					<div class="col-5">
						Assigned: ';
				if ($result['assigned_sink'] == "") {
					$tmp_arr .= $result['assigned_sink'];
				} else {
					$tmp_arr .= $result['sink_location'];
				}
				$tmp_arr .= '
					</div>
				</div>
			...
			} else if ($result['sink_status'] == 4) {
				$tmp_arr .= '
				<hr>
				<div class="container d-flex">
					<div class="col-10 text-muted">
						<b>Status: Accessory not needed.</b>
					</div>
				</div>
				<div class="container d-flex">
					<div class="col-12">
	...
	foreach($materialsbyname as $results) {
		...
		$statusMin = min(array_column($results['detail'], 'sink_status'));
		
		foreach($results['detail'] as $result){
			if ($result['job_status'] > 24 || ($result['job_status'] > 11 && $result['pre_order'] == 1)) {
				if ($statusMin == 0 || $statusMin == 1) {
					$status = 'Status: To be assigned/ordered.';
					if($index1 == 0) {
						if ($result['job_status'] > 31 && $result['job_status'] != 39 && $result['job_status'] < 50 && $result['job_status'] != 44 && $result['job_status'] != 40) {
							$ud_status = new project_action;
				...
				if ($statusMin == 2) {
					$status = 'Status: accessory ordered. Est. delivery ';
					if($index2 == 0) {
						if ($result['job_status'] > 31 && $result['job_status'] != 39 && $result['job_status'] < 50 && $result['job_status'] != 44 && $result['job_status'] != 41) {
							$ud_status = new project_action;
							$pid = $result['pid'];
				...
				if ($statusMin > 2) {
					$status = 'Status: Accessory On Hand';
					if($index3 == 0) {
						if ($result['job_status'] > 31 && $result['job_status'] != 39 && $result['job_status'] < 50 && $result['job_status'] != 44 && $result['job_status'] != 42) {
							$ud_status = new project_action;
							$pid = $result['pid'];
...
if ($action=="get_materials_pull_list") {
	unset($_POST['action']);
	$results = "";
	$search = new materials_action;
...
// 		);
	...
	function show_pull_head($result) {
		...
		if ($result['mat_selected'] == 0) {
			$head_arr .= '
				<div id="' . $result['pid'] . '" class="btn btn-sm btn-warning" onClick="material_selected(' . $result['pid'] . ');"><span class="d-none d-lg-block">Selected </span><i class="fas fa-check-circle"></i></div>';
		}
				...
				if ($result['job_status'] > 24 && $result['job_status'] != 26) {
					if ($result['mat_ready'] == 0) {
						if ($result['mat_selected'] == 1) {
							$fourth_tab .= '<div class="container light-green">';
						} else {
							$fourth_tab .= '<div class="container">';
						}
						if ( !($result['install_date'] == '2200-01-01') ) {
...
// 		);
	...
	foreach($sink_array as $results) {
		...
		$first_tab .= ' <hr class="blue-gradient" style="height:10px">';
		$first_tab .= '	<div id="byMat' . $dateDay . '">';
		$first_tab .= '<style media="print">.d-print-none {display:none;}</style>';
		$first_tab .= '	<div class="w-100 d-flex">';
		$first_tab .= '		<div class="col-12">';
		$first_tab .= '		  <h3>' . $dateDay . ' <i class="fas fa-print text-success d-print-none" style="cursor:pointer" onClick="CallPrint(' . "'byMat" . $dateDay . "'" . ')"></i> <span class="d-none d-print-block">Accessories Pull List - printed: ' . date("Y-m-d h:i:sa") . '</span></h3>';
		$first_tab .= '		</div></div>';
...
//		}
	...
	foreach($sink_array as $results) {
		$second_tab .= ' <hr class="blue-gradient" style="height:10px">';
		$dateDay = $results['install_date'];
		$second_tab .= '	<div id="byJob' . $dateDay . '">';
		$second_tab .= '<style media="print">.d-print-none {display:none;}</style>';
		$second_tab .= '	<div class="w-100 d-flex">';
		$second_tab .= '		<div class="col-12">';
		$second_tab .= '		  <h3>' . $dateDay . ' <i class="fas fa-print text-success d-print-none" style="cursor:pointer" onClick="CallPrint(' . "'byJob" . $dateDay . "'" . ')"></i> <span class="d-none d-print-block">Accessories Pull List - printed: ' . date("Y-m-d h:i:sa") . '</span></h3>';
		$second_tab .= '		</div></div>';
	...
	echo $first_tab . ':::' . $second_tab;
}

if ($action=="get_po_to_order") {
	unset($_POST['action']);
	$first_tab = '<div class="container">';
	$search = new materials_action;
	$mat_array = $search->get_po_mats_needed();
	foreach($mat_array as $results) {
		$jobs_array = $search->get_po_mats_jobs($results['color']);
		$string_jobs = '';
		$jobs_list = '';
		foreach($jobs_array as $r) {
			$jobs_list .= $r['pid'] . ',';
			$string_jobs .= '<i class="pl-5 pr-1 far fa-calendar-alt text-success" style="cursor:pointer" onClick="matOrderedBulk(\'' . $r['pid'] . '\',\'' . $results['color'] . '\')"></i><i class="far fa-hand-holding-box text-warning" title="Mark Materials as Here" style="cursor:pointer" onclick="matOnHandBulk(' . $r['pid'] . ',\'' . addslashes($r['install_name']) . '\')"></i> <!--<i class="fas fa-ban text-danger" style="cursor:pointer" title="Place Materials on Hold" onclick="mat_hold_modal(' . $_SESSION['id'] . ',' . $r['pid'] . ',' . $r['pid'] . ')"></i>--> - <a class="';
		...
		$first_tab .= '<div class="col-12">';
		$first_tab .= '<div class="btn btn-primary float-right" style="cursor:pointer" onClick="matOnHandBulk(\'' . $jobs_list . '\',\'' . $results['color'] . '\')"><i class="far fa-hand-holding-box"></i></div>';
		$first_tab .= '<div class="btn btn-success float-right" style="cursor:pointer" onClick="matOrderedBulk(\'' . $jobs_list . '\',\'' . $results['color'] . '\')"><i class="far fa-calendar-alt"></i></div>';
...
if ($action=="templates_list") {
	...
	function template_item($results) {
		...
		$template_item .= '<div id="' . $results['id'] . '" class="btn btn-sm btn-primary w-100" style="cursor:pointer" onClick="viewThisProject(' .  $results['id'] . ',' . $results['uid'] . ');"><i class="fas fa-eye"></i></div>';
		$template_item .= '</div>';
		$template_item .= '</div>';
		$template_item .= '<hr>';
		...
		<div class="w-100 btn purple-gradient">
			<div class="row">
				...
				<div class="col-md-4 h5">
					...
					?>
						<div class="btn btn-sm btn-danger my-0" style="cursor:pointer" onClick="appReject(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>)"><i class="fas fa-times-octagon text-warning"></i></div>
					<?
					}
					?>
					<div class="btn btn-sm btn-success" style="cursor:pointer" style="cursor: pointer" onClick="mngrApproveLoss(<?= $results['id']; ?>,1,<?= $price_tax ?>,<?= $_SESSION['id']; ?>)"><i class="fas fa-check"></i> Approve</div>
				</div>
			</div>
		</div>
		<?
...
if ($action=="invoice_list") {
	...
	foreach($get_invoices->get_invoices() as $results) {
		...
		$invBtn = '<div class="btn btn-sm btn-success float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',91)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Invoiced" style="cursor:pointer"><i class="fas fa-file-export"></i></div>';
		$uncolBtn = '<div class="btn btn-sm btn-danger float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',98)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Uncollectable" style="cursor:pointer"><i class="fas fa-file-excel"></i></div>';
		$legalBtn = '<div class="btn btn-sm btn-warning float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',97)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Sent to Legal/collections" style="cursor:pointer"><i class="fas fa-balance-scale-right"></i></div>';
		$dispBtn = '<div class="btn btn-sm btn-warning float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',94)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Invoice disputed" style="cursor:pointer"><i class="fas fa-question-circle"></i></div>';
		$ppaidBtn = '<div class="btn btn-sm btn-success float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',93)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Invoice partially paid" style="cursor:pointer"><i class="fas fa-star-half-alt"></i></div>';
		$paidBtn = '<div class="btn btn-sm btn-primary float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',92)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Invoice paid in full" style="cursor:pointer"><i class="fas fa-dollar-sign"></i></div>';
		$holdBtn = '<div class="btn btn-sm btn-danger float-right d-print-none" onClick="statusChangeInv('. $_SESSION['id'] . ',' . $results['id'] . ',99)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Invoice on Hold" style="cursor:pointer"><i class="fas fa-times-octagon"></i></div>';
			...
			echo '<div class="row">';
				...
				echo '<div class="col-md-7 h6 text-left">';
					if ($results['order_num'] == '') {
						...
						echo $results['order_num'] . ' - ';
					}
				echo $results['job_name'] . '</div>';
				echo '<div class="col-md-3 h6">';
					echo '<div class="btn btn-sm btn-primary" style="cursor: pointer" onClick="viewThisProject(' . $results['id'] . ',' . $results['uid'] . ')"><i class="far fa-eye"></i> View</div>';
				echo '</div>';
			echo '</div>';
		echo '</div>';
	}
	...
	foreach($get_entries->get_programming($_SESSION['id']) as $results) {
		if ($results['job_status'] != 32) {
			if ($todayLeft > 0) {
				...
				$date = $date->format('m/d');
				?>" onClick="viewThisProject(<?= $results['id']; ?>,<?= $results['uid']; ?>)" style="cursor:pointer">
					<div class="row">
...
function get_fab_files_new($uid, $pid) {
	$folderPath = base_dir . "job-files/" . $uid . "/" . $pid . "/fab/";
	if (file_exists($folderPath)) {
		$fileList = array_diff(scandir($folderPath), array('..', '.'));
		$return = '';
...
if ($action=="saw_list") {
	...
	$html .= '<div class="progress-bar progress-bar-striped" role="progressbar" aria-valuenow="' . $compPercent . '" aria-valuemin="0" aria-valuemax="100" style="width:' . $compPercent . '%">' . $compPercent . '%</div>';
	$html .= '</div>';
	$html .= '<h3 class="text-primary">';
	if ($_SESSION['access_level'] == 1 || $_SESSION['id'] == 2224) {
		$html .=  $todayCount  . '<sup>SqFt</sup> of ';
		if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14) {
			$html .=  '<input type="number" step="50" class="input-lg text-right limitControl" onClick="change_limit(this)" style="width:100px" data-control="saw_sqft" value="' . $todayLimit . '">';
		} else {
			$html .=  $todayLimit;
		}
		$html .=  '<sup>SqFt</sup> - ';
	}
	$html .= $compPercent  . '% Complete for Today</h3>';
	$html .= '</div>';
	echo $html;
	$results = "";
	unset($_POST['action']);
	...
	foreach($get_entries->get_saw($_SESSION['id']) as $results) {
		if ($results['job_status'] == 51) {
			if ($todayLeft > 0) {
				$thisPercent = number_format((100/$todayLimit)*$results['job_sqft'], 2, '.', ',');
				$html = '<hr>';
				$html .= '<div class="w-100 btn pb-0 ';
				if ($results['job_status'] == 44 || $results['job_status'] == 50) {
					$html .= 'btn-muted mdb-color lighten-5 text-dark';
				...
				$html .= $date;
				$html .= '</div>';
				$html .= '<div class="col-md-5 h5 text-left">' . $results['order_num'] . ' - ' . $results['job_name'] . '</div>';
				$html .= '<div class="col-md-5 h5">';
				$html .= '<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="viewThisProject(' . $results['id'] . ',' . $results['uid'] . ')"><i class="fas fa-eye"></i> View</div>';
				if ($results['job_status'] < 51) {
					$html .= '<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="statusChange(' . $_SESSION['id'] . ',' . $results['id'] . ',51)"><i class="fas fa-arrow-square-right"></i> Start</div>';
				} elseif ($results['job_status'] == 51) {
					$html .= '<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="statusChange(' . $_SESSION['id'] . ',' . $results['id'] . ',52)"><i class="fas fa-check"></i> Complete</div>';
				}
				$html .= '<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="jobHold(' . $_SESSION['id'] . ',' . $results['id'] . ',' . $results['job_status'] . ')"><i class="fas fa-hand-paper text-danger"></i> <span class="text-danger">Hold</span></div>';
				$html .= '</div>';
				$html .= '</div>';
				$html .= '<hr>';
				$html .= '<div class="row">';
				if ($results['job_status'] < 50 && $results['job_status'] != 49 && $results['job_status'] != 32 && $results['prog_ready'] == 0 && $results['mat_ready'] == 1) {
					$html .= '<p>Waiting on programming</p>';
				} elseif ($results['job_status'] < 50 && $results['job_status'] != 49 && $results['job_status'] != 32 && $results['prog_ready'] == 1 && $results['mat_ready'] == 0) {
					$html .= '<p>Waiting on Materials</p>';
				} else {
					$fab_files = get_fab_files_new($results['uid'], $results['id']);
					$html .= $fab_files;
				}
				$html .= '</div>';
				$html .= '</div>';
	...
	foreach($get_entries->get_saw($_SESSION['id']) as $results) {
		if ($results['job_status'] != 51 && $results['job_status'] != 52 && $results['job_status'] != 53) {
			if ($todayLeft > 0) {
				 $thisPercent = number_format((100/$todayLimit)*$results['job_sqft'], 2, '.', ',');
				$html = '<hr>';
				$html .= '<div class="w-100 btn pb-0 ';
				...
				$html .= $thisPercent . '%';
				if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14) {
					$html .= ' ' . $results['zip'] . ' ' . $date;
				}
				$html .= '</div>';
				$html .= '<div class="col-md-5 h5 text-left">';
				if ($results['urgent'] == 1) {
					$html .= '<span class="text-danger" title="911 Job!">';
				} elseif ($results['first_stop'] == 1 || $results['am'] == 1) {
					$html .= '<span class="text-primary" title="First Stop!">';
				} else {
					$html .= '<span>';
				}
				$html .= $results['order_num'] . '</span> - ' . $results['job_name'] . '</div>';
				$html .= '<div class="col-md-5 h5">';
				$html .= '<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="viewThisProject(' . $results['id'] . ',' . $results['uid'] . ')"><i class="fas fa-eye"></i> View</div>';
				...
				$html .= '<hr>';
				$html .= '<div class="row">';
				if ($results['job_status'] != 44 && $results['job_status'] != 50 && $results['prog_ready'] == 0 && $results['mat_ready'] == 1) {
					$html .= '<p>Status: ' . $results['status'] . ' - Waiting on programming</p>';
				} elseif ($results['job_status'] != 44 && $results['job_status'] != 50 && $results['prog_ready'] == 1 && $results['mat_ready'] == 0) {
					$html .= '<p>Status: ' . $results['status'] . ' - Waiting on Materials</p>';
				} elseif ($results['job_status'] != 44 && $results['job_status'] != 50 && $results['prog_ready'] == 0 && $results['mat_ready'] == 0) {
					$html .= '<p>Status: ' . $results['status'] . '</p>';
				} else {
					$html .= get_fab_files_new($results['uid'], $results['id']);
				}
				$html .= '</div>';
				$html .= '</div>';
				$todayLeft = $todayLeft - $results['job_sqft'];
...
<?
						if ($results['job_status'] == 52) {
?>
							<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="statusChange(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,53)"><i class="fas fa-truck"></i> To CNC</div>
<?	
						}
?>
						<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="jobHold(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,<?= $results['job_status'] ?>)"><i class="fas fa-hand-paper text-danger"></i> <span class="text-danger">Hold</span></div>
					</div>
				</div>
				<hr>
				<div class="row">
...
?>
							<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="statusChange(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,62)"><i class="fas fa-check"></i> Complete</div>
<?
						}
?>
						<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="jobHold(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,<?= $results['job_status'] ?>)"><i class="fas fa-hand-paper text-danger"></i> <span class="text-danger">Hold</span></div>
...
<?
	...
	foreach($get_entries->get_cnc($_SESSION['id']) as $results) {
		if ($results['job_status'] != 61 && $results['job_status'] != 62 && $results['job_status'] != 63) {
			if ($todayLeft > 0) {
				 $thisPercent = number_format((100/$todayLimit)*$results['job_sqft'], 2, '.', ',');
?>
			<hr>
...
?>
				...
				<div class="row">
<?
							if ($results['job_status'] == 53 || $results['job_status'] == 60 || $results['job_status'] == 69) {
								get_fab_files($results['uid'], $results['id']);
							} else {
								echo $results['status'];
							}
...
<?
						if ($results['job_status'] == 62) {
?>
							<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="statusChange(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,63)"><i class="fas fa-truck"></i> To Polishing</div>
<?	
						}
?>
						<div class="btn btn-sm btn-primary my-0" style="cursor:pointer" onClick="jobHold(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,<?= $results['job_status'] ?>)"><i class="fas fa-hand-paper text-danger"></i> <span class="text-danger">Hold</span></div>
	...
	function install_item($results) {
		...
		if ($results['install_team'] == 0) {
			if ($_SESSION['access_level'] == 1) {
				$install_item .=	'<p class="text-muted">Unassigned <i class="fas fa-user-plus text-primary" style="cursor:pointer" onClick="instAssignModal(' . $results['id'] . ')"></i>';
				if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14 || $_SESSION['id'] == 1448) {
					$install_item .= '<i class="fas fa-calendar-alt text-primary" style="cursor:pointer"  onclick="dc_modal(' . $results['id'] . ')"></i> ';
				}
				$install_item .=	'</p>';
//				$install_item .= "	<select class='mdb-select md-form colorful-select dropdown-primary border-0 m-0 select" . $results['id'] . "' onchange='assign_install(" . $results['id'] . ");'>";
...
if ($action == "update_install_data") {
	...
	if (!isset($_POST['remnant'])) {
		$_POST['remnant'] = "0";
	}
	if (!isset($_POST['no_mats'])) {
		$_POST['no_mats'] = "0";
	}
...
if ($action=="get_inst_for_update") {
	...
	foreach($array as $result) {
		...
		$return_string .= 'remnant::' . $result['remnant'] . '||';
		$return_string .= 'no_mats::' . $result['no_mats'] . '||';
		$return_string .= 'client_level::' . $result['client_level'] . '||';
...
if ($action=="view_selected_pjt") {
	...
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document" style="margin-top: 7rem;">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="container accountTable">
	...
	foreach($search->project_data_fetch($_POST) as $results) {
		...
		$html .= '<div class="btn btn-primary float-right mr-3  d-print-none" onClick="' . $printThis . '" style="cursor:pointer"><i class="fas fa-print"></i>&nbsp;&nbsp; ';
		if ($job_status < 22) {
			$html .= 'ESTIMATE';
		} elseif ($job_status < 85 && $job_status != 89) {
			$html .= 'QUOTE';
		...
		$html .= '</div></div>';
		$html .= '<h2 class="float-right text-primary d-print-none"><em>' . $results['order_num'] . ' - ';
		if ($results['repair'] == 1) {
		...
		if ($results['clientAccessLevel'] < 11 || $results['uid'] == 1786 || $results['uid'] == 1678) {
			$html .= '<h2 class="d-print-none">Internal Job:</h2>';
		...
		if ($_SESSION['access_level'] < 4) {
			...
			if ($results['clientCompany'] != '') { 
				$html .= $results['clientCompany'] . '</b></h6>';
				if ($results['clientFname'] != '') {
					$html .= '<h6> &emsp;&emsp;FAO: ' . $results['clientFname'] . ' ' . $results['clientLname'] . ',&ensp;</h6>';
				}
			} else {
				$html .= $results['clientFname'] . ' ' . $results['clientLname'] . '</b>,&ensp;</h6>';
			}
			$html .= '<h6> &emsp;&emsp;' . $results['cAdd1'] . ',&ensp;</h6>';
			if ($results['cAdd2'] != '') {
				$html .= '<h6> &emsp;&emsp;' . $results['cAdd2'] . ',&ensp;</h6>';
			}
			$html .= '
				<h6> &emsp;&emsp;' . $results['cCity'] . ',&ensp; ' . $results['cState'] . '&ensp; ' . $results['cZip'] . '</h6>';
				if ($results['cEmail'] != '') {
					$html .= '<h6> &emsp;&emsp;<a href="mailto:' . $results['cEmail'] . '">' . $results['cEmail'] . '</a></h6>';
				}
				if ($results['cPhone'] != '') {
					$html .= '<h6> &emsp;&emsp;<a href="tel:' . $results['cPhone'] . '">' . $results['cPhone'] . '</a></h6>';
				}
			$html .= '</div>
			<div class="col-6"><h6><b>Ship to: ' . $results['contact_name'] . '</b></h6>
				<h6> &emsp;&emsp;' . $results['address_1'] . ',&ensp;</h6>';
			if ($results['address_2'] != '') {
				$html .= '<h6> &emsp;&emsp;' . $results['address_2'] . ',&ensp;</h6>';
			}
			$html .= '<h6> &emsp;&emsp;' . $results['city'] . ',&ensp; ' . $results['state'] . '&ensp; ' . $results['zip'] . '</h6>';
			if ($results['contact_email'] != '') { 
				$html .= '<h6> &emsp;&emsp;<a href="mailto:' . $results['contact_email'] . '">' . $results['contact_email'] . '</a></h6>';
			}
			if ($results['contact_number'] != '') { 
				$html .= '<h6> &emsp;&emsp;<a href="tel:' . $results['contact_number'] . '">' . $results['contact_number'] . '</a></h6>';
			}
			$html .= '</div></div>';
		}
		...
		$payrollYes = '<div class="btn btn-sm btn-primary float-right d-print-none" onClick="payroll_status(1)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Approve Payroll" style="cursor:pointer"><i class="fas fa-envelope-open-dollar"></i></div>';
		$payrollNo = '<div class="btn btn-sm btn-danger float-right d-print-none" onClick="payroll_status(0)" style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="Deny Payroll" style="cursor:pointer"><i class="fas fa-envelope-open-dollar"></i></div>';
		if ($_SESSION['id'] == 1) {
			$html .= $payrollYes;
			$html .= $payrollNo;
		}
		...
		if ($_SESSION['access_level'] < 4) {
			...
			if ($results['job_status'] == 23) {
				...
				if ($approval == 0) {
					...
					$html .= '<div class="btn btn-sm btn-warning float-right d-print-none" onClick="statusChange('. $_SESSION['id'] . ',' . $results['id'] . ',24); console.log($(\'.accountTable\').html())" style="cursor:pointer"><i class="fas fa-check"></i> Quote to Alter</div>';
				} else {
					$html .= '<div class="btn btn-sm btn-success float-right d-print-none" onClick="request_approval('. $_SESSION['id'] . ',' . $results['id'] . ',' . $results['uid'] . ')" style="cursor:pointer"><i class="far fa-question-square"></i> Request Approval</div>';
		...
		if ($_SESSION['access_level'] == 1 || $_SESSION['access_level'] == 10) {
			...
			if ($results['job_status'] == 83) {
				...
				$html .= '<div class="btn btn-sm btn-success float-right d-print-none" onClick="select_installers_modal(' . $results['id'] . ',' . $results['job_sqft'] . ",'" . $results['install_date'] . "'" . ',1);" style="cursor:pointer"><i class="fas fa-check"></i> Install Complete</div>';
		...
		if ($_SESSION['access_level'] == 1 && $_SESSION['id'] != 2224 && $_SESSION['id'] != 1283 && $_SESSION['id'] != 1451 && $_SESSION['id'] != 1452 && $_SESSION['id'] != 1469) {
			$html .= '<select id="changeStatus" onChange="statusChange('. $_SESSION['id'] . ',' . $results['id'] .',this.value)" class="mdb-select md-form colorful-select dropdown-primary float-right d-print-none col-12 col-md-4 col-lg-2 d-print-none">';
			$html .= $statList;
			$html .= '</select>';
//			$html .= '<div id="progressStatus" class="w-100 d-print-none">';
		...
		if ($_SESSION['id'] == 1 || $_SESSION['id'] == 14) {
			$html .= '<div id="asgn_inst" class="btn btn-sm btn-warning d-inline ml-2 float-right d-print-none" onClick="select_installers_modal(' . $results['id'] . ',' . $results['job_sqft'] . ",'" . $results['install_date'] . "'" . ',0);" style="cursor:pointer">Select Installers <i class="fas fa-users"></i></div>';
		}
	...
	$html .= '	<hr class="row d-print-none">';
	...
	$html .= '	<div class="row d-print-none">
					...
					<div class="col-2">
						<div id="uploadmulti" class="btn btn-sm btn-primary" onclick="upload_multi();" style="cursor:pointer">Upload</div>
					</div>
				</div>';
	...
	if (file_exists($folderPath)) {
		...
		if (!empty($fileList)) {
			...
			$html .= '<div id="attachments" class="col-12 d-print-none"><h4>Template Files</h4><div class="row"><div class="col-md-12"><div id="mdb-lightbox-ui"></div><div class="mdb-lightbox">';
		}
		foreach($fileList as $filename) {
			if (strpos($filename, '.') !== false) {
...
//				}
	...
	if($_SESSION['id'] == 1 || $_SESSION['id'] == 14 || $_SESSION['id'] == 1867) {
		$html .= '<hr><div class="d-print-none w-100">';
		$html .= '	<h3 class="d-inline text-primary">Materials Overview </h3>';

		$html .= '	<table class="table table-sm table-striped table-bordered">';
		$html .= ' 		<tr><th>Install</th><th>Qty</th><th>Item</th><th>Hold Order</th><th>Supplier</th><th>Lot</th><th>Assigned</th><th>Status</th></tr>';

		$get_tied_project = new materials_action;
		$tied = $get_tied_project->get_tied_project($pid);
		$options = '';
		foreach($tied as $tr) {
			$options .= '<tr>';
			$options .= '<td>' . $tr['install_name'] . '</td>';
			$options .= '<td class="text-center">' . $tr['item_count'] . '</td>';
			$options .= '<td>';
			if ($tr['category_type'] > 4) {
				$options .= $tr['category_id'] . ' - ';
			}
			$options .= $tr['category_name'];
			if ($tr['remnant'] == 1) {
				$options .= ' (partial)';
			}
			$options .= '</td>';
			$options .= '<td>' . $tr['tie_hold'] . '</td>';
			$options .= '<td>' . $tr['supplier'] . '</td>';
			$options .= '<td>' . $tr['lot'] . '</td>';
			$options .= '<td>';
			if ($tr['location'] > '') {
				$options .= $tr['location'];
			} else if ($tr['min_req'] > 0 && $tr['tie_hold'] < 1){
				$options .= 'Stock';
			}
			$options .= '</td>';
			$options .= '<td>';
			if ($tr['tie_status'] == 1) {
				$options .= 'To Order';
			} else if ($tr['tie_status'] == 2) {
				$options .= 'Ordered';
			} else if ($tr['tie_status'] == 3) {
				$options .= 'Here';
			} else if ($tr['tie_status'] == 4) {
				$options .= 'Ready';
			} else if ($tr['tie_status'] == 5) {
				$options .= 'On Location';
			} else if ($tr['tie_status'] == 6) {
				$options .= 'To Order';
			} 
...
?>
			...
			foreach( $instS->sink_data_fetch($_POST) as $r ) {
				...
				$endSink = $sinkPrice + $r['cutout_price'];
			...
			<?
				if (($_SESSION['access_level'] == 1 || $_SESSION['access_level'] == 6) && ($job_status == 44 || $job_status > 49)) {
			?>

				<i class="fas fa-wrench float-left text-danger" style="cursor: pointer" onClick="adjust_material_open(<?= $_SESSION['id'] ?>,<?= $results['id'] ?>,'<?= $results['lot'] ?>','<?= $results['assigned_material'] ?>')"></i>
...
//		$html .= '</div>';
	...
	if($_SESSION['id'] == 1 || $_SESSION['id'] == 14 || $_SESSION['id'] == 1867) {
		$html .= '<hr><div class="d-print-none w-100">';
		$html .= '	<h3 class="d-inline text-primary">Tied Materials </h3>';

		$html .= '	<div class="btn btn-success ml-2" onClick="open_tie_modal()" style="cursor:pointer">Add <i class="fas fa-plus"></i></div>';
		$get_install_tied = new materials_action;
		$tied = $get_install_tied->get_install_tied($iid);
		$html .= '	<table class="table table-sm table-striped table-bordered">';
		$html .= ' 		<tr><th class="text-center">Qty</th><th>Item</th><th>Hold Order</th><th>Supplier</th><th>Lot</th><th>Assigned</th><th>Status</th></tr>';
...
// LEE UPDATE
	...
	if (($job_status < 30) || $_SESSION['access_level'] == 1) {
		$html .= '	<div class="btn-floating btn-sm btn-warning text-center" onClick="$(' . $modalName . ').modal(' . $modalAction . ')" style="cursor:pointer"><i class="fas fa-info p-absolute" style="position: absolute; margin-left: -3px; top: 10px;"></i></div>';
	...
	foreach( $instR->pieces_data_fetch($_POST) as $r ) {
		$html .= '	<div class="row">';
		$html .= '		<h4 class="d-inline col-12 text-warning text-uppercase">';
		if (($job_status < 30) || $_SESSION['access_level'] == 1) {
	...
	foreach( $instS->sink_data_fetch($_POST) as $r ) {
		...
		if ($r['sink_name'] > '') {
			...
			$programming .= 'Accessory: ' . $r['faucet_name'] . '<br>';
		}
		...
		if ($r['sink_name'] > '') {
			...
			if ($r['sink_onsite'] == 1) {
				$programming .= 'Sink at job site.<br>';
			}
			if ($r['sink_location'] != '') {
				$programming .= 'Location: ' . $r['sink_location'] . '<br>';
			}
			if ($r['assigned_sink'] != '') {
				$programming .= 'Assigned: ' . $r['assigned_sink'] . '<br>';
			}

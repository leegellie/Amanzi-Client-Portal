				...
				success: function(response){
					...
					$.each(extra_projects, function(key, value){
						...
						if (value['temp_first_stop'] == 1) {
							mText = {
								icon: iconBase3,
								text: value['template_date'] + ' 1st Stop',
								bg: 'yellow'
							}
						} else if (value['temp_am'] == 1) {
							mText = {
								icon: iconBase3,
								text: value['template_date'] + ' AM',
								bg: 'yellow'
							}
						} else if (value['temp_pm'] == 1) {
							mText = {
								icon: iconBase3,
								text: value['template_date'] + ' PM',
								bg: 'green'
							}
						} else {
							mText = {
								icon: iconBase3,
								text: value['template_date'],
								bg: 'yellow'
							}
						}
						...
						geocoder.geocode( { 'address': address}, function(results, status) {
							...
							if (status == google.maps.GeocoderStatus.OK) {
								...
								var html_str = "<div style = 'width:200px;min-height:40px'>";
									...
									html_str +=   "Job: <b>" + value['job_name'] + "</b></p>";
									...
									if (selectedTeam == value['template_team']) {html_str += 'selected'};
...
//										html_str += "<div id='myForm'>";
//										html_str +=    "<div>";
//										html_str +=     "<button type='' onclick ='editThisPjt("+value['id']+")'>Assign</button>";
//										html_str +=    "</div>";
//										html_str +=  "</div>";
//										html_str += "</div>";
...
//										html_str += "</div>";
					...
					$.each( result, function( key, value ) {
						...
						joblist_str[teamID] += '<li class="list-group-item"><div class="btn btn-primary btn-sm mr-2 float-left" onClick="targetMap('+latitude+','+longitude+')"><i class="fas fa-bullseye"></i></div><a class="text-primary" target="_blank" onClick="' + modalFunct + '">' + value['job_name'] + '<br><span style="color:#555;text-shadow:none;font-weight:normal;cursor:pointer">' + value['address_1'] + ', ' + value['city'] + ', ' + value['state'] + ', ' + value['zip'] + '</span></a></li>';
...
modalPull += '					<div class="row"><h1>Job: <span class="text-primary">'+value['job_name']+ '</span></h1></div>';
...
modalPull += 						' ' + value['zip'];
modalPull += 					'</b></p></div>';
modalPull += '					<form class="row" id="form' + value['id'] + '"><select class="form-control col-9" onchange="assign_select(' + value['id'] + ');">';
								<?php 
								foreach ($rows as $row){ ?>
									var selectedTeam = <?php echo $row['temp_team_id']; ?>;
modalPull += '						<option id="contactChoice<?php echo $row['temp_team_id']; ?>" value="<?php echo $row['temp_team_id']; ?>" '
if (selectedTeam == value['template_team']) {modalPull += 'selected'};
...
//           var point_item_assign = new google.maps.LatLng(latPull, longPull);
//           nodes_team[n].push(point_item_assign);
					//joblist_str[teamID] += '<li>'+value['job_name']+'</li>'
					...
					joblist_html[teamID] += '<li class="list-group-item"><div class="btn btn-primary btn-sm mr-2 float-left" onClick="targetMap('+latPull+','+longPull+')"><i class="fas fa-bullseye"></i></div><a class="text-primary" style="cursor:pointer" target="_blank" onClick="' + modalFunct + '">' + value['job_name'] + '<br><span style="color:#555;text-shadow:none;font-weight:normal">' + value['address_1'] + ', ' + value['city'] + ', ' + value['state'] + ', ' + value['zip'] + '</span></a></li>';
...
<body style="margin: 0; padding: 0; overflow-x: hidden; background:none !important">
	<div class="loading hidden h-100 w-100 m-0 position-absolute" style="z-index: 3; touch-action: pan-x pan-y;background: #0000004a url(../CARloader.gif) no-repeat fixed center;"></div>
			...
			<div class="container" style="overflow-y:auto; overflow-x:hidden; max-height:100vh">
				...
				<div class="row">
					<div class="col-12 pr-4 team_section">
						...
						foreach ($rows as $row){ 
							$color = '';
							if ($row['temp_team_id'] == 1) {
								$color = '#000000';
							} else if ($row['temp_team_id'] == 2) {
								$color = '#003366';
							} else if ($row['temp_team_id'] == 3) {
								$color = '#184832';
							} else if ($row['temp_team_id'] == 4) {
								$color = '#e85050';
							} else if ($row['temp_team_id'] == 5) {
								$color = '#ff8800';
							} else if ($row['temp_team_id'] == 6) {
								$color = '#a4d1f0';
							} else if ($row['temp_team_id'] == 7) {
								$color = '#417b3e';
							} else {
								$color = '#CCCCCC';
							}
						?>
						<div class="row team<?= $row['temp_team_id'] ?>">
							<div class="col-sm-1 team_no">
								<?
								if ($row['temp_team_id'] > 0) {
								?>
							...
							<div class="col-sm-9 team_no">
								<span class="pl-1"><?php echo $row['temp_team_name']; ?></span>
							</div>
							<div class="col-sm-2">
								<?
								if ($row['temp_team_id'] > 0) {
								?>
								<button class="btn btn-primary mt-1" onclick="display_route(<?php echo $row['temp_team_id']; ?>)">Route</button>
								<?
								}
								?>
							</div>
